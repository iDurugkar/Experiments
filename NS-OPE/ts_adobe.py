...
mseFile = 'AdobeData/mse_test-0.7_ts_%d_%d.txt' % (start_i, end_i)
with open(mseFile, 'w') as rf:
    rf.writelines(['Point\tMSE\n'])
absFile = 'AdobeData/abs_test-0.7_ts_%d_%d.txt' % (start_i, end_i)
...
for j in range(start_i/100, end_i/100):
    ...
    try:
        res = sm.tsa.ARMA(dataset, (3, 0)).fit(trend="nc")
